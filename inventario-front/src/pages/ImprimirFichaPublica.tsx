...
// Misma planilla que ImprimirFicha.tsx, pero pegándole a los endpoints
// públicos (sin login) — a la que se llega desde la vista pública del QR
export default function ImprimirFichaPublica() {
  const { id } = useParams();
  ...
  useEffect(() => {
    Promise.all([
      api.get(`/publico/equipos/${id}`),
      api.get(`/publico/equipos/${id}/qr`),
    ]).then(([ficha, qr]) => {
      setEquipo(ficha.data.equipo);
      setHistorial(ficha.data.historial);
      setEncabezado(ficha.data.encabezado);
  ...
  if (error) {
    return <div className="min-h-screen flex items-center justify-center text-baja text-sm font-bold">{error}</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-fondo text-tinta">
      <div className="print:hidden bg-primario p-4 flex justify-between items-center">
        <button onClick={() => navigate(`/publico/equipos/${id}`)} className="flex items-center gap-2 text-white text-sm font-bold cursor-pointer">
